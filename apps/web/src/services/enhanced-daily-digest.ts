...
import { findStaleContacts, findHighImportancePersons } from '~/data-access/persons';
...
export class EnhancedBriefGeneratorService {
  ...
  private async fetchEnhancedBriefData(): Promise<EnhancedBriefData> {
    ...
    const [
      ...
      this.options.includeCommitments
        ? findCommitmentsWithPerson(this.userId, {
            status: ['pending', 'in_progress'],
          }).then((all) => all.filter((c) => c.dueDate && c.dueDate < new Date()))
        : Promise.resolve([]),
      this.options.includeCommitments
        ? findCommitmentsWithPerson(this.userId, {
            status: ['pending', 'in_progress'],
            ...
            return all.filter((c) => c.dueDate && c.dueDate >= now && c.dueDate <= weekLater);
          })
        : Promise.resolve([]),
  ...
  private generateEnhancedBriefContent(data: EnhancedBriefData, briefDate: string): string {
    ...
    sections.push(
      `- **Emails:** ${data.emails.length} (${data.emailAnalysis.summary.needsResponse} need response)`
    );
    ...
    if (this.options.includeCommitments && data.upcomingCommitments.length > 0) {
      ...
      for (const c of data.upcomingCommitments.slice(0, 5)) {
        ...
        const dueStr = c.dueDate
          ? c.dueDate.toLocaleDateString('en-US', {
              weekday: 'short',
              month: 'short',
              day: 'numeric',
            })
    ...
    if (data.emails.length === 0) {
      ...
      if (data.emailAnalysis.grouped.awaitingReply.length > 0) {
        sections.push(
          `### Awaiting Reply: ${data.emailAnalysis.grouped.awaitingReply.length} emails`
        );
        sections.push('');
      }
