...
export class MeetingBriefingService {
  ...
  async generateBriefingsForUpcomingMeetings(
    ...
    const events = await fetchEventsForDailyBrief(
      this.integration!,
  ...
  private async gatherAttendeeInfo(
    ...
    for (const attendee of eventAttendees) {
      ...
      if (person) {
        ...
        const personCommitments = allCommitments.filter(
  ...
  private formatBriefingContent(data: {
    ...
    if (attendees.length > 0) {
      ...
      for (const attendee of attendees) {
        ...
        const roleCompany = [attendee.role, attendee.company]
          .filter(Boolean)
          .join(" at ");
 ...
 */
export async function processPendingMeetingBriefings(
