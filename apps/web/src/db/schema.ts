...
export const statementRun = pgTable(
  "statement_run",
  {
    ...
    userId: text("user_id")
      ...
      .references(() => user.id, { onDelete: "cascade" }),
    status: text("status")
      .$type<"running" | "completed" | "failed" | "mfa_required">()
      .notNull(),
...
export const person = pgTable(
  "person",
  {
    ...
    emails: jsonb("emails").$type<string[]>().$default(() => []), // All known emails
    name: text("name"),
    phone: text("phone"),
    // Organization info
    role: text("role"),
    ...
    // Categorization
    domain: text("domain").$type<PersonDomain>().$default(() => "business"),
...
export const notification = pgTable(
  "notification",
  {
    ...
    urgency: text("urgency").$type<"high" | "medium" | "low">().$default(() => "medium"),
    channels: jsonb("channels").$type<NotificationChannel[]>().$default(() => ["in_app"]),
    // Status tracking
...
export const notificationPreferences = pgTable(
  "notification_preferences",
  {
    ...
    preferences: jsonb("preferences").$type<NotificationPreferencesConfig>().$default(() => ({})),
    // Quiet hours
...
export const ingestionEvent = pgTable(
  "ingestion_event",
  {
    ...
    // Event type
    eventType: text("event_type").$type<"new_email" | "email_update" | "calendar_event" | "calendar_update">().notNull(),
    // External reference
...
export const privacySettings = pgTable(
  "privacy_settings",
  {
    ...
    userId: text("user_id")
      ...
      .unique()
      .references(() => user.id, { onDelete: "cascade" }),
    allowCloudAI: boolean("allow_cloud_ai").$default(() => true).notNull(),
    excludedDomains: jsonb("excluded_domains").$type<PersonDomain[]>().$default(() => []),
    excludedPersonIds: jsonb("excluded_person_ids").$type<string[]>().$default(() => []),
    excludedEmailDomains: jsonb("excluded_email_domains").$type<string[]>().$default(() => []),
    redactPatterns: jsonb("redact_patterns").$type<string[]>().$default(() => []),
    createdAt: timestamp("created_at")
      .$defaultFn(() => new Date())
      .notNull(),
    updatedAt: timestamp("updated_at")
  ...
  (table) => [
    index("idx_privacy_settings_user_id").on(table.userId),
  ]
);
...
export const backfillJob = pgTable(
  "backfill_job",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
    status: text("status").$type<BackfillJobStatus>().$default(() => "pending").notNull(),
    sourceType: text("source_type").$type<BackfillSourceType>().notNull(),
    startDate: timestamp("start_date").notNull(),
    endDate: timestamp("end_date").notNull(),
    progress: jsonb("progress").$type<BackfillProgress>().$default(() => ({ processed: 0, total: 0 })),
    error: text("error"),
    // Statistics
    personsCreated: integer("persons_created").$default(() => 0),
    interactionsCreated: integer("interactions_created").$default(() => 0),
...
export type UpdateCommitmentData = Partial<Omit<CreateCommitmentData, "id" | "userId" | "createdAt">>;
...
export type UpdateNotificationData = Partial<Omit<CreateNotificationData, "id" | "userId" | "createdAt">>;

// Notification Preferences types
export type NotificationPreferences = typeof notificationPreferences.$inferSelect;
export type CreateNotificationPreferencesData = typeof notificationPreferences.$inferInsert;
export type UpdateNotificationPreferencesData = Partial<Omit<CreateNotificationPreferencesData, "id" | "userId" | "createdAt">>;
...
export type UpdateMeetingBriefingData = Partial<Omit<CreateMeetingBriefingData, "id" | "userId" | "createdAt">>;
...
export type UpdateIngestionEventData = Partial<Omit<CreateIngestionEventData, "id" | "userId" | "createdAt">>;
...
export type UpdateRelationshipData = Partial<Omit<CreateRelationshipData, "id" | "userId" | "createdAt">>;

// Domain Rule types
export type DomainRule = typeof domainRule.$inferSelect;
export type CreateDomainRuleData = typeof domainRule.$inferInsert;
export type UpdateDomainRuleData = Partial<Omit<CreateDomainRuleData, "id" | "userId" | "createdAt">>;

// Privacy Settings types
export type PrivacySettings = typeof privacySettings.$inferSelect;
export type CreatePrivacySettingsData = typeof privacySettings.$inferInsert;
export type UpdatePrivacySettingsData = Partial<Omit<CreatePrivacySettingsData, "id" | "userId" | "createdAt">>;

// Backfill Job types
export type BackfillJob = typeof backfillJob.$inferSelect;
export type CreateBackfillJobData = typeof backfillJob.$inferInsert;
export type UpdateBackfillJobData = Partial<Omit<CreateBackfillJobData, "id" | "userId" | "createdAt">>;
...
export type ActionLogStatus = "pending_approval" | "approved" | "rejected" | "executed" | "failed" | "reversed";
...
export const actionLog = pgTable(
  "action_log",
  {
    ...
    actionTypeId: text("action_type_id")
      ...
      .references(() => actionType.id, { onDelete: "cascade" }),
    authorityLevel: text("authority_level").$type<AuthorityLevel>().notNull(),
    status: text("status").$type<ActionLogStatus>().$default(() => "pending_approval").notNull(),
...
export type UpdateAuthoritySettingData = Partial<Omit<CreateAuthoritySettingData, "id" | "userId" | "createdAt">>;
