...
import {
  ...
  useCreateDomainRule,
  useDeleteDomainRule,
} from "~/hooks/use-domain-rules";
import type { DomainRuleType, PersonDomain, DomainRule } from "~/db/schema";

const RULE_TYPE_LABELS: Record<DomainRuleType, { label: string; icon: typeof Mail; description: string }> = {
  email_domain: {
    label: "Email Domain",
    icon: AtSign,
      ...
      <CardContent className="space-y-6">
        ...
        {rules.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm">
              Add rules to automatically classify your contacts
            </p>
          </div>
        )}
      </CardContent>
  ...
  return (
    <div className="flex items-center justify-between rounded-md border p-3">
      <div className="flex items-center gap-3">
        ...
        <Badge className={DOMAIN_COLORS[rule.domain as PersonDomain]}>
          {rule.domain}
        </Badge>
      </div>
      <Button
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={domain} onValueChange={(v) => setDomain(v as PersonDomain)}>
                ...
                <SelectContent>
                  {(["family", "business", "job", "personal", "other"] as PersonDomain[]).map((d) => (
                    <SelectItem key={d} value={d}>
                      <div className="flex items-center gap-2">
                        <div className={`h-2 w-2 rounded-full ${DOMAIN_COLORS[d].split(" ")[0]}`} />
...
function QuickAddSuggestions({ existingRules }: { existingRules: DomainRule[] }) {
  ...
  const suggestions = [
    { ruleType: "email_domain" as DomainRuleType, pattern: "@gmail.com", domain: "personal" as PersonDomain, label: "Gmail → Personal" },
    { ruleType: "email_domain" as DomainRuleType, pattern: "@outlook.com", domain: "personal" as PersonDomain, label: "Outlook → Personal" },
    { ruleType: "email_domain" as DomainRuleType, pattern: "@yahoo.com", domain: "personal" as PersonDomain, label: "Yahoo → Personal" },
    { ruleType: "keyword" as DomainRuleType, pattern: "invoice", domain: "business" as PersonDomain, label: "Invoice → Business" },
    { ruleType: "keyword" as DomainRuleType, pattern: "family", domain: "family" as PersonDomain, label: "Family → Family" },
