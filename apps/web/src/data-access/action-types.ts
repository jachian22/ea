 ...
 */
export async function createActionType(data: CreateActionTypeData): Promise<ActionType> {
  const [newActionType] = await database.insert(actionType).values(data).returning();
 ...
 */
export async function findActionTypeById(id: string): Promise<ActionType | null> {
  const [result] = await database.select().from(actionType).where(eq(actionType.id, id)).limit(1);
 ...
 */
export async function findActionTypeByName(name: string): Promise<ActionType | null> {
 ...
 */
export async function findActionTypesByCategory(category: ActionCategory): Promise<ActionType[]> {
  return database.select().from(actionType).where(eq(actionType.category, category));
}

/**
 * Find action types by risk level
 */
export async function findActionTypesByRiskLevel(riskLevel: RiskLevel): Promise<ActionType[]> {
  return database.select().from(actionType).where(eq(actionType.riskLevel, riskLevel));
}
...
export async function deleteActionType(id: string): Promise<boolean> {
  const [deleted] = await database.delete(actionType).where(eq(actionType.id, id)).returning();
