...
export function registerCalendarTools(server: McpServer, userId: string) {
  ...
  server.tool(
    ...
    "Get calendar events for a date range. Returns events with attendee context and meeting links.",
    {
      ...
      endDate: z.string().optional().describe("End date (ISO format). Defaults to end of start date."),
  ...
  server.tool(
    ...
    "Find free time slots in the calendar based on constraints.",
    {
      ...
      startDate: z.string().optional().describe("Start of search range (ISO format). Defaults to now."),
      endDate: z.string().optional().describe("End of search range (ISO format). Defaults to 7 days from start."),
          ...
          content: [{
            ...
            text: JSON.stringify({
              ...
              freeSlots: freeSlots.slice(0, 20).map(slot => ({
                ...
                date: slot.start.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' }),
...
function findFreeSlots(
  ...
  while (current < rangeEnd) {
    ...
    const dayBusy = busyPeriods.filter(b =>
      b.start < effectiveEnd && b.end > effectiveStart
    );
