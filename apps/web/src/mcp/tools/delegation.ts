...
export function registerDelegationTools(server: McpServer, userId: string) {
  ...
  server.tool(
    ...
    {
      domain: z.enum(["business", "job", "family", "personal", "other"]).optional().describe("Filter by domain"),
  ...
  server.tool(
    ...
    "Create a new task delegation - assign a task to someone else.",
    {
      ...
      context: z.string().optional().describe("Additional context or instructions for the delegate"),
    },
          ...
          content: [{
            ...
            text: JSON.stringify({
              ...
              delegations: delegations.map(d => ({
                ...
                isOverdue: d.dueDate && d.dueDate < now &&
                  (d.status === "pending" || d.status === "in_progress"),
