...
export function registerBriefingTools(server: McpServer, userId: string) {
  ...
  server.tool(
    ...
    {
      date: z.string().optional().describe("Date to get brief for (ISO format). Defaults to today."),
    },
    async ({ date }) => {
      try {
  ...
  server.tool(
    ...
    {
      startDate: z.string().optional().describe("Start of week (ISO format). Defaults to start of current week."),
    },
    async ({ startDate }) => {
      try {
  ...
  server.tool(
    ...
    async ({ domain }) => {
      try {
        ...
        const domainCommitments = commitments.filter(c =>
          c.personId && domainPersonIds.has(c.personId)
        );

        // Get recent interactions
        const interactions = await findInteractionsWithPerson(userId, 100);
        const domainInteractions = interactions.filter(i =>
          i.personId && domainPersonIds.has(i.personId)
        ).slice(0, 20);

        // Calculate stats
        const overdueCommitments = domainCommitments.filter(c =>
        ...
        const staleContacts = people.filter(p =>
          !p.lastContactAt || p.lastContactAt < thirtyDaysAgo
        );
