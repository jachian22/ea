...
import type { CalendarEventData, EmailData, EnrichedBriefData } from '../db/schema.js';
...
async function executeClaudeCLI(
  ...
  return new Promise((resolve) => {
    ...
    // Use spawn with arguments array - NO shell to prevent command injection
    const child = spawn(CLAUDE_CLI_PATH, ['--print', prompt], {
      env: {
        ...process.env,
        CI: 'true',
      },
